function [Result,interHeD,interKL,interJS]=Fuzzy_weight_information(data)
% tic
% clc
% clear
% close all
...
%     [x,y]=eig(Mutiadj(:,:,i));
%     weight=abs(diag(y));
% [weight]=eigenvector(Mutiadj(:,:,i));
%     weights(i,:)=weight';
    
    [ weight,GML ]= get_multi_local(MutiDist(:,:,i),N,q);
     weights(i,:)=weight(1,:);
   
% weights(i,:)=mycoreness(MutiDist(:,:,i));

% [weight] = KsG_Cenctrity(MutiDist(:,:,i),MutiDist(:,:,i),Ks(i,:));
%  weights(i,:)=weight;
end
